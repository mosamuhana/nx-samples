...
export class AuthProcessService implements ISignInProcess, ISignUpProcess {
  onSuccessEmitter = new EventEmitter<any>();
      ...
      switch (provider) {
        ...
        case AuthProvider.EmailAndPassword:
          if (credentials) {
            ...
            signInResult = result as UserCredential;
          }
          break;
  ...
  public updateProfile(name: string, photoURL?: string): Promise<void> {
    return this.afa.currentUser.then((user: User | null) => {
      if (user) {
        if (!photoURL) {
  ...
  public parseUserInfo(user: User): UserInfo {
    ...
    if (user.providerData && user.providerData.length > 0) {
      const providers = user.providerData.filter(x => !!x) as UserInfo[];
  ...
  showToast(message: string) {
    if (message) {
      this.snackBar.open(
        message,
        this.matSnackBarConfig.duration ? undefined : "OK"
      );
    }
  }
