  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['template']) {
      if (changes['template'].currentValue == null) {
        this._initTemplate(this.defaultTemplate);
      } else {
        this._initTemplate(changes['template'].currentValue);
      }
    }
  }

  ngOnInit(): void {
    this._initTemplate(this.template);
  }

  private _initTemplate(template?: TemplateRef<any>): void {
    this.verifyEmailTemplate = template ?? this.defaultTemplate;
    /*
